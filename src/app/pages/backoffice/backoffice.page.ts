...
export class BackofficePage implements OnInit, OnDestroy {
  ...
  private handlePdfFile(file: File) {
    ...
    if (file.size > 700 * 1024) {
      alert(
        `File troppo grande (${(file.size / 1024 / 1024).toFixed(2)} MB). \nIl limite è 1MB. Comprimi il PDF.`,
      );
      return;
    }
  ...
  filterStartups(startups: Startup[]): Startup[] {
    ...
    return startups.filter((s) =>
      s.name.toLowerCase().includes(this.startupSearchTerm.toLowerCase()),
    );
  }
  ...
  async addStartup() {
    ...
    this.newStartupLogo = await this.dbService.uploadFile(
      this.newStartupLogo,
      `startups/${this.newStartupName}/logo`,
    );
  ...
  async updateStartup() {
    ...
    this.selectedStartup.logoUrl = await this.dbService.uploadFile(
      this.selectedStartup.logoUrl!,
      `startups/${this.selectedStartup.name}/logo`,
    );
  ...
  deleteStartup(id: string) {
    if (
      confirm('Eliminare definitivamente questa startup e tutti i dipendenti?')
    ) {
  ...
  async saveEmployee() {
    if (!this.selectedStartup || !this.selectedStartup.id || !this.empFormName)
      return;
    this.empFormImage = await this.dbService.uploadFile(
      this.empFormImage,
      `startups/${this.selectedStartup.name}/employees/${this.empFormName}`,
    );
    ...
    if (this.editingEmployee) {
      // MODIFICA ESISTENTE
      await this.dbService.updateEmployeeDetails(
        this.selectedStartup.id,
        this.editingEmployee,
        newEmpData,
      );
    } else {
      // AGGIUNGI NUOVO
      await this.dbService.addEmployeeToStartup(
        this.selectedStartup.id,
        newEmpData,
      );
    }
  ...
  async deleteEmployee(emp: Employee) {
    ...
    if (confirm('Rimuovere dipendente?')) {
      await this.dbService.removeEmployeeFromStartup(
        this.selectedStartup.id,
        emp.name,
      );
    }
  ...
  private handleFile(file: File, target: string) {
    ...
    reader.onload = () => {
      ...
      if (target === 'editStartup' && this.selectedStartup)
        this.selectedStartup.logoUrl = res;
      if (target === 'employee') this.empFormImage = res;
      if (target === 'thirdParty') this.newThirdPartyLogo = res;
      if (target === 'editThirdParty' && this.selectedThirdParty)
        this.selectedThirdParty.logoUrl = res;
      if (target === 'tpEmployee') this.newTpEmpImage = res;
      if (target === 'newSupplier') this.newSupplierLogo = res;
      if (target === 'editSupplier' && this.selectedSupplier)
        this.selectedSupplier.logoUrl = res;
  ...
  async addSupplier() {
    ...
    this.newSupplierLogo = await this.dbService.uploadFile(
      this.newSupplierLogo,
      `suppliers/${this.newSupplierName}/logo`,
    );
  ...
  async deleteSupplier(id: string) {
    if (confirm('Eliminare fornitore?')) {
      this.dbService.deleteSupplier(id);
      this.deselectSupplier();
    }
  }
  ...
  async updateSupplier() {
    ...
    this.selectedSupplier.logoUrl = await this.dbService.uploadFile(
      this.selectedSupplier.logoUrl!,
      `suppliers/${this.selectedSupplier.name}/logo`,
    );
  ...
  async addThirdParty() {
    ...
    this.newThirdPartyLogo = await this.dbService.uploadFile(
      this.newThirdPartyLogo,
      `thirdParties/${this.newThirdPartyName}/logo`,
    );
    await this.dbService.addThirdParty({
      name: this.newThirdPartyName,
      logoUrl: this.newThirdPartyLogo,
      employees: [],
    });
    this.newThirdPartyName = '';
    this.newThirdPartyLogo = '';
  }

  async deleteThirdParty(id: string) {
    if (confirm('Eliminare Utente Terzo?')) {
      this.dbService.deleteThirdParty(id);
      this.deselectThirdParty();
    }
  ...
  async addTpEmployee() {
    ...
    await this.dbService.addEmployeeToThirdParty(
      this.selectedThirdParty.id!,
      emp,
    );
  ...
  async deleteTpEmployee(emp: Employee) {
    ...
    if (confirm('Rimuovere persona?'))
      this.dbService.removeEmployeeFromThirdParty(
        this.selectedThirdParty.id!,
        emp.name,
      );
  }
  ...
  async checkTpEmployeeOut(employee: Employee, thirdParty: ThirdParty) {
    if (confirm("Confermi l'uscita della persona?")) {
      this.dbService.updateTpEmployeeStatus(
        thirdParty.id!,
        employee.name,
        'OUT',
      );
  ...
  filterThirdParties(thirdParties: ThirdParty[]): ThirdParty[] {
    ...
    return thirdParties.filter((s) =>
      s.name.toLowerCase().includes(this.thirdPartySearchTerm.toLowerCase()),
    );
  }
  async updateThirdParty() {
    if (!this.selectedThirdParty || !this.selectedThirdParty.id) return;
    const name = this.selectedThirdParty.name?.trim();
    if (!name) {
      alert('Nome obbligatorio.');
      return;
    }
    const data: Partial<ThirdParty> = {
      name,
    };
    if (this.selectedThirdParty.logoUrl !== undefined) {
      data.logoUrl = this.selectedThirdParty.logoUrl;
    }
    await this.dbService.updateThirdParty(this.selectedThirdParty.id, data);
  ...
  async saveTpEmployee() {
    ...
    if (this.editingEmployee) {
      await this.dbService.updateTpEmployeeDetails(
        this.selectedThirdParty.id!,
        this.editingEmployee,
        {
          name: this.newTpEmpName,
